...
networks:
  digroup-network:
    driver: bridge
"@
$dockerCompose | Out-File -FilePath "$TestDir\docker-compose.yml" -Encoding UTF8
...
"@
$prometheusConfig | Out-File -FilePath "$TestDir\prometheus.yml" -Encoding UTF8
...
Get-Content "$TestDir\.env" | ForEach-Object {
    if ($_ -match '^([^=]+)=(.*)$') {
