// DIGroup - Refactor your thinking
...
const confDir = path.join(app.getPath("home"), ".config", "digroup");
...
// Устанавливаем название приложения для macOS
app.setName("DIGroup");

app.setPath("userData", app.getPath("userData") + "-Electron"); // `~/.config` 下 Electron 相关文件夹名称改为 `DIGroup-Electron` https://github.com/siyuan-note/siyuan/issues/3349
...
try {
    ...
    console.error(e);
    require("electron").dialog.showErrorBox("创建配置目录失败 Failed to create config directory", "DIGroup需要在用户家目录下创建配置文件夹（~/.config/digroup），请确保该路径具有写入权限。\n\nDIGroup needs to create a configuration folder (~/.config/digroup) in the user's home directory. Please make sure that the path has write permissions.");
    app.exit();
}
...
const initMainWindow = () => {
    ...
    currentWindow.webContents.userAgent = "DIGroup/" + appVer + " Electron " + currentWindow.webContents.userAgent;

    // set proxy
    net.fetch(getServer() + "/api/system/getNetwork", {method: "POST"}).then((response) => {
        return response.json();
    }).then((response) => {
        // Безопасная проверка наличия proxy в ответе
        if (response && response.data && response.data.proxy && response.data.proxy.scheme && response.data.proxy.host && response.data.proxy.port) {
            setProxy(`${response.data.proxy.scheme}://${response.data.proxy.host}:${response.data.proxy.port}`, currentWindow.webContents).then(() => {
                loadMainWindow();
            }).catch((err) => {
                writeLog("set proxy failed: " + err.message);
                loadMainWindow(); // Продолжаем загрузку даже если proxy не установлен
            });
        } else {
            writeLog("proxy not available in response, loading without proxy");
            loadMainWindow(); // Загружаем без proxy если он не доступен
        }
    }).catch((err) => {
        writeLog("get network config failed: " + err.message);
        loadMainWindow(); // Продолжаем загрузку даже если не удалось получить настройки сети
    });
    
    function loadMainWindow() {
        // В Electron приложении загружаем через check-auth
        // Страница check-auth автоматически перенаправит после аутентификации
        // Для автоматической аутентификации в dev режиме используем AccessAuthCode из kernel
        const authUrl = getServer() + "/check-auth?to=" + encodeURIComponent("/stage/build/app/");
        currentWindow.loadURL(authUrl + "&v=" + new Date().getTime());
        
        // Устанавливаем обработчик для автоматической аутентификации в dev режиме
        if (isDevEnv) {
            currentWindow.webContents.once("did-finish-load", () => {
                // Если загрузилась страница check-auth, автоматически отправляем AccessAuthCode
                currentWindow.webContents.executeJavaScript(`
                    (function() {
                        const authCodeInput = document.querySelector('input[type="password"], input[name="authCode"]');
                        if (authCodeInput && !authCodeInput.value) {
                            authCodeInput.value = "b226ba0f30a134fe9245792118bca202";
                            const submitBtn = document.querySelector('button[type="submit"], .b3-button--text');
                            if (submitBtn) {
    ...
    currentWindow.webContents.on("did-finish-load", () => {
        let digroupOpenURL = process.argv.find((arg) => arg.startsWith("digroup://"));
        if (digroupOpenURL) {
            ...
            setTimeout(() => { // 等待界面js执行完毕
                writeLog(digroupOpenURL);
                currentWindow.webContents.send("digroup-open-url", digroupOpenURL);
            }, 2000);
        }
    });
...
const initKernel = (workspace, port, lang) => {
    return new Promise(async (resolve) => {
        ...
        const kernelName = "win32" === process.platform ? "DIGroup-Kernel.exe" : "SiYuan-Kernel";
        const kernelPath = path.join(appDir, "kernel", kernelName);
        if (!fs.existsSync(kernelPath)) {
            showErrorWindow("⚠️ 内核程序丢失 Kernel program is missing", `<div>内核程序丢失，请重新安装 DIGroup，并将 DIGroup 内核程序加入杀毒软件信任列表。</div><div>The kernel program is not found, please reinstall DIGroup and add DIGroup Kernel program into the trust list of your antivirus software.</div><div><i>${kernelPath}</i></div>`);
            bootWindow.destroy();
            resolve(false);
            return;
        }
        ...
        if (!isDevEnv || workspaces.length > 0) {
            ...
            kernelProcess.on("close", (code) => {
                ...
                if (0 !== code) {
                    ...
                    switch (code) {
                        case 20:
                            errorWindowId = showErrorWindow("⚠️ 数据库被锁定 The database is locked", "<div>数据库文件正在被其他进程占用，请检查是否同时存在多个内核进程（DIGroup Kernel）服务相同的工作空间。</div><div>The database file is being occupied by other processes, please check whether there are multiple kernel processes (DIGroup Kernel) serving the same workspace at the same time.</div>");
                            break;
                        case 21:
                        ...
                        case 24: // 工作空间已被锁定，尝试切换到第一个打开的工作空间
                            ...
                            errorWindowId = showErrorWindow("⚠️ 工作空间已被锁定 The workspace is locked", "<div>该工作空间正在被使用，请尝试在任务管理器中结束 DIGroup-Kernel 进程或者重启操作系统后再启动 DIGroup。</div><div>The workspace is being used, please try to end the DIGroup-Kernel process in the task manager or restart the operating system and then start DIGroup.</div>");
                            break;
                        case 25:
                            ...
                            break;
                        case 26:
                            errorWindowId = showErrorWindow("🚒 已成功避免潜在的数据损坏<br>Successfully avoid potential data corruption", "<div>工作空间下的文件正在被第三方软件（比如同步网盘、杀毒软件等）打开占用，继续使用会导致数据损坏，DIGroup 内核已经安全退出。<br><br>请将工作空间移动到其他路径后再打开，停止同步盘同步工作空间，并将工作空间加入杀毒软件信任列表。</div><hr><div>The files in the workspace are being opened and occupied by third-party software (such as synchronized network disk, antivirus software, etc.), continuing to use it will cause data corruption, and the DIGroup Kernel is already safe shutdown.<br><br>Move the workspace to another path and open it again, stop the network disk to sync the workspace, and add the workspace to the antivirus software trust list.</div>");
                            break;
                        case 0:
                            break;
                        default:
                            errorWindowId = showErrorWindow("⚠️ 内核因未知原因退出 The kernel exited for unknown reasons", `<div>DIGroup 内核因未知原因退出 [code=${code}]，请尝试重启操作系统后再启动 DIGroup。如果该问题依然发生，请检查杀毒软件是否阻止 DIGroup 内核启动。</div><div>DIGroup Kernel exited for unknown reasons [code=${code}], please try to reboot your operating system and then start DIGroup again. If occurs this problem still, please check your anti-virus software whether kill the DIGroup Kernel.</div>`);
                            break;
                    }
        ...
        for (; ;) {
            try {
                ...
                if (14 < ++count) {
                    ...
                    showErrorWindow("⚠️ 获取内核服务端口失败 Failed to get kernel serve port", "<div>获取内核服务端口失败，请确保程序拥有网络权限并不受防火墙和杀毒软件阻止。如果 kernel  запущен отдельно, проверьте файл ~/.config/digroup/port.json</div><div>Failed to get kernel serve port, please make sure the program has network permissions and is not blocked by firewalls and antivirus software. If kernel is running separately, check ~/.config/digroup/port.json file.</div>");
                    bootWindow.destroy();
                    resolve(false);
                    return;
                }
...
for (let i = argStart; i < process.argv.length; i++) {
    ...
    if (arg.startsWith("--workspace=") || arg.startsWith("--openAsHidden") || arg.startsWith("--port=") || arg.startsWith("digroup://")) {
        // 跳过内置参数
...
app.whenReady().then(() => {
    ...
    ipcMain.on("siyuan-open-window", (event, data) => {
        ...
        win.webContents.userAgent = "DIGroup/" + appVer + " https://b3log.org/siyuan Electron " + win.webContents.userAgent;
    ...
    if (firstOpen) {
        ...
        let language = "ru_RU"; // Установлен русский язык по умолчанию
