...
<div class="backlinkMList fn__flex-1"></div>`;
    ...
    private searchBacklinks(init = false) {
        ...
        fetchPost("/api/ref/getBacklink2", {
            ...
            // #region agent log
            fetch('http://127.0.0.1:7242/ingest/51c402ed-a6bc-49c1-9938-f2186d8909a5',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'Backlink.ts:507',message:'Backlink API response handler',data:{code:response.code,hasData:!!response.data,hasBacklinks:!!response.data?.backlinks,hasBackmentions:!!response.data?.backmentions,backlinksCount:response.data?.backlinks?.length||0,backmentionsCount:response.data?.backmentions?.length||0,linkRefsCount:response.data?.linkRefsCount||0,mentionsCount:response.data?.mentionsCount||0},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'D'})}).catch(()=>{});
            // #endregion
            element.classList.remove("fn__rotate");
            if (response.code === 0 && response.data) {
                ...
                console.error("Backlink API error:", response);
                // Рендерим пустые данные при ошибке
                this.render({
                    box: "",
                    backlinks: [],
                    backmentions: [],
