...
func indexBox(boxID string) {
	logging.LogInfof("[BOOT] indexBox() started for box [%s]", boxID)
	box := Conf.Box(boxID)
	if nil == box {
		logging.LogWarnf("[BOOT] Box [%s] not found, skipping index", boxID)
		return
	}

	logging.LogInfof("[BOOT] Listing files for box [%s]...", boxID)
	util.SetBootDetails("Listing files...")
	files := box.ListFiles("/")
	logging.LogInfof("[BOOT] Found %d files in box [%s]", len(files), boxID)
	...
	// Добавляем таймаут для индексации - максимум 60 секунд на блокнот
	indexStartTime := time.Now()
	indexTimeout := 60 * time.Second

	for _, file := range files {
		// Проверяем таймаут перед обработкой каждого файла
		if time.Since(indexStartTime) > indexTimeout {
			logging.LogWarnf("[BOOT] indexBox timeout after 60s for box [%s], stopping index", boxID)
			break
		}
		...
		if nil != invokeErr {
			...
			waitGroup.Done() // Уменьшаем счетчик если не удалось добавить задачу
			continue
		}
	}

	// Добавляем таймаут для waitGroup, чтобы не зависать
	logging.LogInfof("[BOOT] Waiting for index pool to complete (timeout: 60s)...")
	...
	select {
	case <-done:
		logging.LogInfof("[BOOT] Index pool completed")
	case <-time.After(60 * time.Second):
		logging.LogWarnf("[BOOT] indexBox timeout after 60s for box [%s], continuing anyway", boxID)
		// Не блокируем загрузку, продолжаем работу
	}
