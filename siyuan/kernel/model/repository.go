...
func bootSyncRepo() (err error) {
	logging.LogInfof("[BOOT] bootSyncRepo() started")
	if 1 > len(Conf.Repo.Key) {
		logging.LogInfof("[BOOT] Repo.Key is empty, skipping sync")
		autoSyncErrCount++
		planSyncAfter(fixSyncInterval)
	...
	logging.LogInfof("[BOOT] Creating repository...")
	repo, err := newRepository()
	if err != nil {
		logging.LogErrorf("[BOOT] newRepository() failed: %s", err)
		autoSyncErrCount++
		planSyncAfter(fixSyncInterval)
	...
	logging.LogInfof("[BOOT] Repository created")

	logging.LogInfof("[BOOT] Starting boot sync...")
	isBootSyncing.Store(true)

	waitGroup := sync.WaitGroup{}
	var errs []error
	logging.LogInfof("[BOOT] Starting indexRepoBeforeCloudSync in goroutine...")
	waitGroup.Add(1)
	go func() {
		...
		start := time.Now()
		logging.LogInfof("[BOOT] indexRepoBeforeCloudSync started")
	...
	logging.LogInfof("[BOOT] Waiting for goroutines to complete (timeout: 30s)...")
	...
	select {
	case <-done:
		logging.LogInfof("[BOOT] Goroutines completed")
	case <-time.After(30 * time.Second):
		logging.LogWarnf("[BOOT] bootSyncRepo timeout after 30s, continuing anyway")
		// Не блокируем загрузку, продолжаем работу
	}
	if 0 < len(errs) {
		err = errs[0]
		logging.LogErrorf("[BOOT] bootSyncRepo() errors: %v", errs)
	}
	...
	logging.LogInfof("[BOOT] bootSyncRepo() completed")
	return
}
