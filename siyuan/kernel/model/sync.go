...
func BootSyncData() {
	defer logging.Recover()
	logging.LogInfof("[BOOT] BootSyncData() started")

	if Conf.Sync.Perception {
		logging.LogInfof("[BOOT] Connecting sync WebSocket...")
		connectSyncWebSocket()
		logging.LogInfof("[BOOT] Sync WebSocket connected")
	} else {
		logging.LogInfof("[BOOT] Sync.Perception is false, skipping WebSocket")
	}

	logging.LogInfof("[BOOT] Checking sync...")
	if !checkSync(true, false, false) {
		logging.LogInfof("[BOOT] checkSync returned false, skipping sync")
		return
	}
	logging.LogInfof("[BOOT] checkSync passed")

	// Добавляем таймаут для проверки провайдера (может зависать на сетевых запросах)
	logging.LogInfof("[BOOT] Checking provider online (timeout: 5s)...")
	providerCheckDone := make(chan bool, 1)
	var isOnline bool
	go func() {
		isOnline = isProviderOnline(false)
		providerCheckDone <- true
	}()

	select {
	case <-providerCheckDone:
		if !isOnline {
			logging.LogInfof("[BOOT] Provider is offline, skipping sync")
			BootSyncSucc = 1
			util.PushErrMsg(Conf.Language(76), 7000)
			return
		}
		logging.LogInfof("[BOOT] Provider is online")
	case <-time.After(5 * time.Second):
		logging.LogWarnf("[BOOT] Provider check timeout after 5s, skipping sync")
		BootSyncSucc = 1
		return
	}

	logging.LogInfof("[BOOT] Locking sync...")
	lockSync()
	defer unlockSync()
	logging.LogInfof("[BOOT] Sync locked")

	util.IncBootProgress(3, "Syncing data from the cloud...")
	BootSyncSucc = 0
	logging.LogInfof("[BOOT] Starting bootSyncRepo()...")
	...
	if err != nil {
		code = 2
		logging.LogErrorf("[BOOT] bootSyncRepo() failed: %s", err)
	} else {
		logging.LogInfof("[BOOT] bootSyncRepo() completed successfully")
	}
	util.BroadcastByType("main", "syncing", code, Conf.Sync.Stat, nil)
...
func checkSync(boot, exit, byHand bool) bool {
	...
	// Проверки подписки отключены - приложение опенсорс, синхронизация всегда доступна
