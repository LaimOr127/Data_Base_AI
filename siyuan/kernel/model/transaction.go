...
func flushTx(tx *Transaction) {
	...
	if txErr := performTx(tx); nil != txErr {
		...
		default:
			...
			logging.LogErrorf("transaction failed [%d]: %s\n  tx [%s]", txErr.code, txErr.msg, txData)
			// Не завершаем процесс, отправляем уведомление пользователю
			util.PushErrMsg(fmt.Sprintf("Transaction failed: %s", txErr.msg), 5000)
		}
	}
...
func performTx(tx *Transaction) (ret *TxErr) {
	...
	if !isLargeInsert {
		...
		for _, op := range tx.DoOperations {
			// Добавляем информацию о пользователе в операцию для аудита
			if tx.UserLogin != "" {
				if op.Context == nil {
					op.Context = make(map[string]interface{})
				}
				op.Context["userLogin"] = tx.UserLogin
			}
			switch op.Action {
			case "create":
				ret = tx.doCreate(op)
...
func (tx *Transaction) doMove(operation *Operation) (ret *TxErr) {
	...
	refreshUpdated(srcNode)
	refreshUpdatedBy(srcNode, tx.UserLogin)
	tx.nodes[srcNode.ID] = srcNode
	refreshUpdated(srcTree.Root)
	refreshUpdatedBy(srcTree.Root, tx.UserLogin)
	if err = tx.writeTree(srcTree); err != nil {
...
// refreshUpdatedBy добавляет информацию о пользователе, который обновил блок
func refreshUpdatedBy(node *ast.Node, userLogin string) {
	if userLogin != "" {
		node.SetIALAttr("updated-by", userLogin)
		parents := treenode.ParentNodesWithHeadings(node)
		for _, parent := range parents {
			parent.SetIALAttr("updated-by", userLogin)
		}
	}
}
...
type Transaction struct {
	...
	UserLogin      string       `json:"userLogin"` // Пользователь, выполнивший транзакцию
