...
func InitConf() {
	...
	if "" != util.Lang {
		...
		if "" == Conf.Lang {
			...
			if userLang, err := locale.Detect(); err == nil {
				...
				logging.LogDebugf("check device locale failed [%s], using default language [ru_RU]", err)
				util.Lang = "ru_RU"
				Conf.Lang = util.Lang
			}
		}
	...
	if nil == Conf.Publish {
		...
		// Если Publish уже загружен, убеждаемся, что Auth инициализирован
		if Conf.Publish.Auth == nil {
			Conf.Publish.Auth = &conf.BasicAuth{
				Enable:   true,
				Accounts: []*conf.BasicAuthAccount{},
	...
	if 0 >= Conf.AI.Ollama.APITemperature || 2 < Conf.AI.Ollama.APITemperature {
		Conf.AI.Ollama.APITemperature = 1.0
	}
	...
	if nil == Conf.AI.Gemini {
		Conf.AI.Gemini = &conf.Gemini{
			...
			APIMaxContexts: 7,
			APITimeout:     30,
			APIModel:       "gemini-1.5-flash", // Используем актуальную модель
			APIBaseURL:     "https://generativelanguage.googleapis.com/v1beta",
			APIKey:         "AIzaSyCby8forvtNvqZYBCJozW-VlI0GuUQrE4A",
		}
	}
	if "" == Conf.AI.Gemini.APIModel {
		Conf.AI.Gemini.APIModel = "gemini-1.5-flash"
	}
	if 0 > Conf.AI.Gemini.APIMaxTokens {
		Conf.AI.Gemini.APIMaxTokens = 0
	}
	if 0 >= Conf.AI.Gemini.APITemperature || 2 < Conf.AI.Gemini.APITemperature {
		Conf.AI.Gemini.APITemperature = 1.0
	}
	...
	if Conf.AI.Ollama != nil && "" != Conf.AI.Ollama.APIBaseURL {
		...
		// Сохраняем конфигурацию, чтобы Gemini был доступен после перезапуска
		Conf.Save()
	}

	// Логирование активного провайдера
	if Conf.AI.Ollama != nil && "" != Conf.AI.Ollama.APIBaseURL {
		logging.LogInfof("Ollama API enabled (default)\n"+
			"    baseURL=%s\n"+
			"    timeout=%ds\n"+
			"    proxy=%s\n"+
			"    model=%s\n"+
			...
			"    temperature=%.1f\n"+
			"    maxContexts=%d",
			Conf.AI.Ollama.APIBaseURL,
			Conf.AI.Ollama.APITimeout,
			Conf.AI.Ollama.APIProxy,
			Conf.AI.Ollama.APIModel,
			Conf.AI.Ollama.APIMaxTokens,
			Conf.AI.Ollama.APITemperature,
			Conf.AI.Ollama.APIMaxContexts)
	} else if Conf.AI.Gemini != nil && "" != Conf.AI.Gemini.APIKey {
		logging.LogInfof("Gemini API enabled (default)\n"+
			"    baseURL=%s\n"+
			"    timeout=%ds\n"+
			"    proxy=%s\n"+
			"    model=%s\n"+
			...
			"    temperature=%.1f\n"+
			"    maxContexts=%d",
			Conf.AI.Gemini.APIBaseURL,
			Conf.AI.Gemini.APITimeout,
			Conf.AI.Gemini.APIProxy,
			Conf.AI.Gemini.APIModel,
			Conf.AI.Gemini.APIMaxTokens,
			Conf.AI.Gemini.APITemperature,
			Conf.AI.Gemini.APIMaxContexts)
	} else if "" != Conf.AI.OpenAI.APIKey {
	...
	// Обеспечиваем, что AccessAuthCode всегда установлен для обязательной авторизации
	if "" == Conf.AccessAuthCode {
		// Генерируем случайный код, если не установлен
		Conf.AccessAuthCode = gulu.Rand.String(32)
		logging.LogWarnf("AccessAuthCode was not set, generated new one: %s", Conf.AccessAuthCode)
		Conf.Save()
	}
...
func InitBoxes() {
	logging.LogInfof("[BOOT] InitBoxes() started")
	blockCount := treenode.CountBlocks()
	initialized := 0 < blockCount
	logging.LogInfof("[BOOT] Block count: %d, initialized: %v", blockCount, initialized)

	// Автоматически создаем блокнот по умолчанию, если блокнотов нет
	logging.LogInfof("[BOOT] Listing notebooks...")
	notebooks, err := ListNotebooks()
	if err != nil {
		logging.LogErrorf("list notebooks failed: %s", err)
	} else if len(notebooks) == 0 {
		// Создаем блокнот по умолчанию
		defaultNotebookName := "Блокнот"
		if Conf.Lang == "en_US" {
			defaultNotebookName = "Notebook"
		}
		boxID, createErr := CreateBox(defaultNotebookName)
		if createErr != nil {
			logging.LogErrorf("create default notebook failed: %s", createErr)
		} else {
			logging.LogInfof("created default notebook [%s] with name [%s]", boxID, defaultNotebookName)
			// Открываем созданный блокнот
			box := &Box{ID: boxID}
			boxConf := box.GetConf()
			boxConf.Closed = false
			box.SaveConf(boxConf)
		}
		// Перечитываем список блокнотов
		notebooks, err = ListNotebooks()
		if err != nil {
			logging.LogErrorf("re-list notebooks failed: %s", err)
		}
	}

	logging.LogInfof("[BOOT] Processing %d opened boxes...", len(Conf.GetOpenedBoxes()))
	for _, box := range Conf.GetOpenedBoxes() {
		logging.LogInfof("[BOOT] Processing box [%s]...", box.ID)
		box.UpdateHistoryGenerated() // 初始化历史生成时间为当前时间

		if !initialized {
			logging.LogInfof("[BOOT] Indexing box [%s]...", box.ID)
			indexBox(box.ID)
			logging.LogInfof("[BOOT] Box [%s] indexed", box.ID)
		}
	}

	logging.LogInfof("tree/block count [%d/%d], notebooks [%d]", treenode.CountTrees(), blockCount, len(notebooks))
	logging.LogInfof("[BOOT] InitBoxes() completed")
}

func IsSubscriber() bool {
	// Всегда подписчик - приложение опенсорс, полный функционал бесплатно
	return true
}
...
func clearPortJSON() {
	...
	portJSON := filepath.Join(util.HomeDir, ".config", "digroup", "port.json")
