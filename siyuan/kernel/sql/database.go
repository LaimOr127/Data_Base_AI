...
func initDBTables() {
	...
	if err != nil {
		logging.LogErrorf("drop table [blocks_fts] failed: %s", err)
	}
	...
	if err != nil {
		// Обрабатываем ошибку fts5 gracefully
		if strings.Contains(err.Error(), "fts5") || strings.Contains(err.Error(), "no such module") {
			logging.LogWarnf("FTS5 module not available, full-text search will be limited: %s", err)
			// Создаем обычную таблицу вместо FTS5
			_, fallbackErr := db.Exec("CREATE TABLE IF NOT EXISTS blocks_fts(id, parent_id, root_id, hash, box, path, hpath, name, alias, memo, tag, content, fcontent, markdown, length, type, subtype, ial, sort, created, updated)")
			if fallbackErr != nil {
				logging.LogErrorf("create fallback blocks_fts table failed: %s", fallbackErr)
			} else {
				logging.LogInfof("Created fallback blocks_fts table without FTS5")
			}
		} else {
			logging.LogErrorf("create table [blocks_fts] failed: %s", err)
		}
	}

	_, err = db.Exec("DROP TABLE IF EXISTS blocks_fts_case_insensitive")
	if err != nil {
		logging.LogErrorf("drop table [blocks_fts_case_insensitive] failed: %s", err)
	}
	...
	if err != nil {
		// Обрабатываем ошибку fts5 gracefully
		if strings.Contains(err.Error(), "fts5") || strings.Contains(err.Error(), "no such module") {
			logging.LogWarnf("FTS5 module not available, case-insensitive search will be limited: %s", err)
			// Создаем обычную таблицу вместо FTS5
			_, fallbackErr := db.Exec("CREATE TABLE IF NOT EXISTS blocks_fts_case_insensitive(id, parent_id, root_id, hash, box, path, hpath, name, alias, memo, tag, content, fcontent, markdown, length, type, subtype, ial, sort, created, updated)")
			if fallbackErr != nil {
				logging.LogErrorf("create fallback blocks_fts_case_insensitive table failed: %s", fallbackErr)
			} else {
				logging.LogInfof("Created fallback blocks_fts_case_insensitive table without FTS5")
			}
		} else {
			logging.LogErrorf("create table [blocks_fts_case_insensitive] failed: %s", err)
		}
	}
...
func initHistoryDBTables() {
	historyDB.Exec("DROP TABLE IF EXISTS histories_fts_case_insensitive")
	...
	if err != nil {
		// Обрабатываем ошибку fts5 gracefully, не завершаем процесс
		if strings.Contains(err.Error(), "fts5") || strings.Contains(err.Error(), "no such module") {
			logging.LogWarnf("FTS5 module not available, history search will be limited: %s", err)
			// Создаем обычную таблицу вместо FTS5 для совместимости
			_, fallbackErr := historyDB.Exec("CREATE TABLE IF NOT EXISTS histories_fts_case_insensitive(id, type, op, title, content, path, created)")
			if fallbackErr != nil {
				logging.LogErrorf("create fallback history table failed: %s", fallbackErr)
			} else {
				logging.LogInfof("Created fallback history table without FTS5")
			}
		} else {
			logging.LogErrorf("create table [histories_fts_case_insensitive] failed: %s", err)
			util.PushErrMsg("Ошибка инициализации базы данных истории, проверьте логи", 5000)
		}
	}
}
...
func initAssetContentDBTables() {
	assetContentDB.Exec("DROP TABLE IF EXISTS asset_contents_fts_case_insensitive")
	...
	if err != nil {
		// Обрабатываем ошибку fts5 gracefully
		if strings.Contains(err.Error(), "fts5") || strings.Contains(err.Error(), "no such module") {
			logging.LogWarnf("FTS5 module not available, asset content search will be limited: %s", err)
			// Создаем обычную таблицу вместо FTS5
			_, fallbackErr := assetContentDB.Exec("CREATE TABLE IF NOT EXISTS asset_contents_fts_case_insensitive(id, name, ext, path, size, updated, content)")
			if fallbackErr != nil {
				logging.LogErrorf("create fallback asset_contents_fts_case_insensitive table failed: %s", fallbackErr)
			} else {
				logging.LogInfof("Created fallback asset_contents_fts_case_insensitive table without FTS5")
			}
		} else {
			logging.LogErrorf("create table [asset_contents_fts_case_insensitive] failed: %s", err)
		}
	}
}
