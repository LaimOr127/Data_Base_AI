...
func getBacklink2(c *gin.Context) {
	...
	// #region agent log
	_, hasAuth := c.Get("auth")
	logging.LogInfof("[DEBUG] getBacklink2 called from [%s], hasAuth: %v", util.GetRemoteAddr(c.Request), hasAuth)
	// #endregion

	arg, ok := util.JsonArg(c, ret)
	if !ok {
		// #region agent log
		logging.LogWarnf("[DEBUG] getBacklink2 JsonArg failed")
		// #endregion
		return
	}
	...
	if nil == arg["id"] {
		// #region agent log
		logging.LogWarnf("[DEBUG] getBacklink2 id is nil")
		// #endregion
		return
	}
	...
	// #region agent log
	logging.LogInfof("[DEBUG] getBacklink2 result: boxID=%s, backlinks=%d, backmentions=%d, linkRefsCount=%d, mentionsCount=%d", boxID, len(backlinks), len(backmentions), linkRefsCount, mentionsCount)
	// #endregion
	ret.Data = map[string]interface{}{
		"backlinks":     backlinks,
...
func getBacklink(c *gin.Context) {
	...
	// #region agent log
	_, hasAuth := c.Get("auth")
	logging.LogInfof("[DEBUG] getBacklink called from [%s], hasAuth: %v", util.GetRemoteAddr(c.Request), hasAuth)
	// #endregion

	arg, ok := util.JsonArg(c, ret)
	if !ok {
		// #region agent log
		logging.LogWarnf("[DEBUG] getBacklink JsonArg failed")
		// #endregion
		return
	}
	...
	if nil == arg["id"] {
		// #region agent log
		logging.LogWarnf("[DEBUG] getBacklink id is nil")
		// #endregion
		return
	}
	...
	// #region agent log
	logging.LogInfof("[DEBUG] getBacklink result: boxID=%s, backlinks=%d, backmentions=%d, linkRefsCount=%d, mentionsCount=%d", boxID, len(backlinks), len(backmentions), linkRefsCount, mentionsCount)
	// #endregion
	ret.Data = map[string]interface{}{
		"backlinks":     backlinks,
		"linkRefsCount": linkRefsCount,
