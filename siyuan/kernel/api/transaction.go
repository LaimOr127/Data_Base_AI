...
func performTransactions(c *gin.Context) {
	...
	// Получаем пользователя из сессии для аудита
	userLogin := ""
	sessionData := util.GetSession(c)
	workspaceSession := util.GetWorkspaceSession(sessionData)
	if workspaceSession.UserLogin != "" {
		userLogin = workspaceSession.UserLogin
	}

	for _, transaction := range transactions {
		transaction.Timestamp = timestamp
		transaction.UserLogin = userLogin // Сохраняем пользователя для аудита
	}
