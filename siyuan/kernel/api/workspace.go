...
func setWorkspaceDir(c *gin.Context) {
	...
	// Блокируем только прямые пути к облачным дискам (onedrive, dropbox в пути)
	// Desktop и Documents больше не блокируются, даже если они синхронизируются с iCloud
	pathLower := strings.ToLower(path)
	desktopPath := filepath.Join(util.HomeDir, "Desktop")
	documentsPath := filepath.Join(util.HomeDir, "Documents")
	
	// Разрешаем Desktop и Documents
	isDesktopOrDocuments := strings.HasPrefix(pathLower, strings.ToLower(desktopPath)) || 
		strings.HasPrefix(pathLower, strings.ToLower(documentsPath))
	
	// Проверяем только если это НЕ Desktop или Documents
	if !isDesktopOrDocuments {
