...
func setAI(c *gin.Context) {
	...
	// Валидация настроек Ollama, если они присутствуют
	if ai.Ollama != nil {
		if 5 > ai.Ollama.APITimeout {
			ai.Ollama.APITimeout = 5
		}
		if 600 < ai.Ollama.APITimeout {
			ai.Ollama.APITimeout = 600
		}
		if 0 > ai.Ollama.APIMaxTokens {
			ai.Ollama.APIMaxTokens = 0
		}
		if 0 >= ai.Ollama.APITemperature || 2 < ai.Ollama.APITemperature {
			ai.Ollama.APITemperature = 1.0
		}
		if 1 > ai.Ollama.APIMaxContexts || 64 < ai.Ollama.APIMaxContexts {
			ai.Ollama.APIMaxContexts = 7
		}
		if "" == ai.Ollama.APIBaseURL {
			ai.Ollama.APIBaseURL = "http://localhost:11434"
		}
	}

	// Валидация настроек Gemini, если они присутствуют
	if ai.Gemini != nil {
		if 5 > ai.Gemini.APITimeout {
			ai.Gemini.APITimeout = 5
		}
		if 600 < ai.Gemini.APITimeout {
			ai.Gemini.APITimeout = 600
		}
		if 0 > ai.Gemini.APIMaxTokens {
			ai.Gemini.APIMaxTokens = 0
		}
		if 0 >= ai.Gemini.APITemperature || 2 < ai.Gemini.APITemperature {
			ai.Gemini.APITemperature = 1.0
		}
		if 1 > ai.Gemini.APIMaxContexts || 64 < ai.Gemini.APIMaxContexts {
			ai.Gemini.APIMaxContexts = 7
		}
	}
