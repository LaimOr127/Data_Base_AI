...
func Serve(fastMode bool) {
	...
	ginServer.Use(
		metricsMiddleware(),      // Сбор метрик HTTP запросов
	...
	if err != nil {
		if !fastMode {
			...
			// Пробуем использовать другой порт вместо завершения процесса
			for port := 6807; port <= 6810; port++ {
				altPort := fmt.Sprintf("%d", port)
				if altLn, altErr := net.Listen("tcp", host+":"+altPort); altErr == nil {
					logging.LogWarnf("Port %s unavailable, using alternative port %s", util.ServerPort, altPort)
					ln = altLn
					util.ServerPort = altPort
					err = nil
					break
				}
			}
			if err != nil {
				// Если все порты заняты, логируем ошибку, но не завершаем процесс
				logging.LogErrorf("All ports unavailable, server will retry later")
				util.PushErrMsg("Не удалось запустить сервер, проверьте доступность портов", 10000)
				return
			}
		} else {
			// fast 模式下启动失败则直接返回
	...
	if err = http.Serve(ln, ginServer.Handler()); err != nil {
		if !fastMode {
			logging.LogErrorf("HTTP server error: %s", err)
			// Не завершаем процесс, логируем ошибку
			util.PushErrMsg(fmt.Sprintf("Ошибка HTTP сервера: %s", err), 5000)
		}
	}
}
...
func serveWebSocket(ginServer *gin.Engine) {
	...
	util.WebSocketServer.HandleConnect(func(s *melody.Session) {
		logging.LogInfof("[WS] WebSocket connection attempt from [%s], URI: [%s]", util.GetRemoteAddr(s.Request), s.Request.RequestURI)
		util.IncWSConnections()
		authOk := true
		...
		if "" != model.Conf.AccessAuthCode {
			...
			if err != nil {
				authOk = false
				logging.LogWarnf("[WS] get cookie failed: %s", err)
			} else {
				val := session.Values["data"]
				if nil == val {
					authOk = false
					logging.LogWarnf("[WS] session data is nil")
				} else {
					sess := &util.SessionData{}
					err = gulu.JSON.UnmarshalJSON([]byte(val.(string)), sess)
					if err != nil {
						authOk = false
						logging.LogWarnf("[WS] unmarshal cookie failed: %s", err)
					} else {
		...
		if !authOk {
			if token := model.ParseXAuthToken(s.Request); token != nil {
				...
				if authOk {
					logging.LogInfof("[WS] authenticated via X-Auth-Token")
				}
			}
		}
		...
		if !authOk {
			...
			if authOk {
				logging.LogInfof("[WS] allowing auth page connection")
			}
		}

		// Временно разрешаем подключения от Electron во время загрузки (первые 60 секунд после старта)
		// Это позволяет UI подключиться до авторизации для начальной загрузки
		if !authOk {
			userAgent := s.Request.Header.Get("User-Agent")
			if strings.Contains(userAgent, "Electron") || strings.Contains(userAgent, "DIGroup") {
				// Проверяем, прошло ли менее 60 секунд с момента старта
				// Используем безопасную проверку: если BootTime не инициализирован (zero time), разрешаем подключение
				if util.BootTime.IsZero() || time.Since(util.BootTime) < 60*time.Second {
					logging.LogInfof("[WS] Allowing Electron connection during boot (User-Agent: %s, time since boot: %v)", userAgent, time.Since(util.BootTime))
					authOk = true
				} else {
					logging.LogWarnf("[WS] Electron connection denied - boot window expired (User-Agent: %s, time since boot: %v)", userAgent, time.Since(util.BootTime))
				}
			}
		}
		...
		if !authOk {
			...
			logging.LogWarnf("[WS] closed an unauthenticated session [%s]", util.GetRemoteAddr(s.Request))
			return
		}
		...
		appId, _ := s.Get("app")
		sessionType, _ := s.Get("type")
		logging.LogInfof("[WS] WebSocket connected: app=[%s], id=[%s], type=[%s]", appId, sessionId, sessionType)
	})

	util.WebSocketServer.HandleDisconnect(func(s *melody.Session) {
		util.RemovePushChan(s)
		util.DecWSConnections()
		//sessionId, _ := s.Get("id")
...
func metricsMiddleware() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		util.IncHTTPRequests()
		defer util.DecHTTPRequests()
