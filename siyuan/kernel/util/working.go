...
var (
	...
	BootTime     time.Time        // Время запуска kernel для проверки временных разрешений
)
...
func Boot() {
	BootTime = time.Now() // Сохраняем время запуска для проверки временных разрешений
	initEnvVars()
...
var (
	...
	ShortcutsPath      string        // 用户家目录下的快捷方式目录路径 home/.config/digroup/shortcuts/

	UIProcessIDs = sync.Map{} // UI 进程 ID
)

func initWorkspaceDir(workspaceArg string) {
	userHomeConfDir := filepath.Join(HomeDir, ".config", "digroup")
...
func ReadWorkspacePaths() (ret []string, err error) {
	ret = []string{}
	workspaceConf := filepath.Join(HomeDir, ".config", "digroup", "workspace.json")
