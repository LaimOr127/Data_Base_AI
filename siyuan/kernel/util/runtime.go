...
func HookUILoaded() {
	// Уменьшаем таймаут до 5 секунд, чтобы не блокировать загрузку
	timeout := time.After(5 * time.Second)
	ticker := time.NewTicker(200 * time.Millisecond)
	defer ticker.Stop()

	logging.LogInfof("[BOOT] HookUILoaded started, waiting for UI connection...")

	for !IsUILoaded {
		select {
		case <-timeout:
			logging.LogWarnf("[BOOT] HookUILoaded timeout after 5 seconds, continuing anyway")
			IsUILoaded = true // Продолжаем работу даже если UI не подключился
			return
		case <-ticker.C:
			mainSessions := SessionsByType("main")
			if 0 < len(mainSessions) {
				logging.LogInfof("[BOOT] UI connected with %d main session(s)", len(mainSessions))
				IsUILoaded = true
				return
			}
		}
...
func ReportFileSysFatalError(err error) {
	...
	// Не завершаем процесс, отправляем уведомление
	PushErrMsg("Обнаружена проблема с файловой системой, проверьте логи", 10000)
}
...
func IsCloudDrivePath(workspaceAbsPath string) bool {
	// Более мягкая проверка - разрешаем Desktop и Documents
	// Блокируем только прямые пути к облачным дискам

	// Проверяем только прямые пути к iCloud Mobile Documents
	if isICloudPath(workspaceAbsPath) {
		return true
	}

	// Проверяем известные облачные диски в пути (onedrive, dropbox и т.д.)
	if isKnownCloudDrivePath(workspaceAbsPath) {
		return true
	}

	// Проверка на Windows OneDrive через AvailabilityStatus
	// Но не блокируем Desktop и Documents автоматически
...
func isICloudPath(workspaceAbsPath string) (ret bool) {
	...
	// Блокируем только прямые пути к iCloud Mobile Documents
	iCloudRoot := filepath.Join(HomeDir, "Library", "Mobile Documents")
	if strings.HasPrefix(workspaceAbsPathLower, strings.ToLower(iCloudRoot)) {
		logging.LogWarnf("workspace [%s] is in iCloud path [%s]", workspaceAbsPath, iCloudRoot)
		return true
	}

	// Desktop и Documents больше не блокируются автоматически
	// Проверяем только реальные iCloud синхронизированные папки через WalkWithSymlinks
	WalkWithSymlinks(iCloudRoot, func(path string, d fs.DirEntry, err error) error {
		if !d.IsDir() {
			return nil
		}

		// Проверяем только прямые пути внутри iCloud Mobile Documents
