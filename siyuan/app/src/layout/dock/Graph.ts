    ...
    public searchGraph(focus: boolean, id?: string, refresh = false) {
        ...
        if (this.type === "global") {
            ...
            fetchPost("/api/graph/getGraph", {
                ...
                // #region agent log
                fetch('http://127.0.0.1:7242/ingest/51c402ed-a6bc-49c1-9938-f2186d8909a5',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'Graph.ts:456',message:'Graph API response handler',data:{code:response.code,hasData:!!response.data,hasNodes:!!response.data?.nodes,hasLinks:!!response.data?.links,nodesCount:response.data?.nodes?.length||0,linksCount:response.data?.links?.length||0},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'D'})}).catch(()=>{});
                // #endregion
                if (response.code === 0 && response.data) {
                    ...
                    // #region agent log
                    fetch('http://127.0.0.1:7242/ingest/51c402ed-a6bc-49c1-9938-f2186d8909a5',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'Graph.ts:462',message:'Graph API error',data:{code:response.code,msg:response.msg,hasData:!!response.data},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'B'})}).catch(()=>{});
                    // #endregion
            ...
            fetchPost("/api/graph/getLocalGraph", {
                ...
                if (response.code === 0 && response.data) {
                    ...
                    console.error("Local Graph API error:", response);
                }
            });
        }
    }
