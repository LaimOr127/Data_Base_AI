...
export const fetchPost = (url: string, data?: any, cb?: (response: IWebSocketData) => void, headers?: IObject) => {
    ...
    fetch(url, init).then((response) => {
        // #region agent log
        if (url.includes("/api/graph/") || url.includes("/api/ref/")) {
            fetch('http://127.0.0.1:7242/ingest/51c402ed-a6bc-49c1-9938-f2186d8909a5',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'fetch.ts:35',message:'Graph/Backlink API request',data:{url,status:response.status,statusText:response.statusText,hasCookies:document.cookie.length>0},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'A'})}).catch(()=>{});
        }
        // #endregion
        switch (response.status) {
            ...
            default:
                if (401 == response.status) {
                    // #region agent log
                    if (url.includes("/api/graph/") || url.includes("/api/ref/")) {
                        fetch('http://127.0.0.1:7242/ingest/51c402ed-a6bc-49c1-9938-f2186d8909a5',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'fetch.ts:45',message:'401 Unauthorized for graph/backlink',data:{url,status:response.status},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'A'})}).catch(()=>{});
                    }
                    // #endregion
        ...
        if (url.includes("/api/graph/") || url.includes("/api/ref/")) {
            ...
            fetch('http://127.0.0.1:7242/ingest/51c402ed-a6bc-49c1-9938-f2186d8909a5',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'fetch.ts:58',message:'Graph/Backlink API response',data:{url,code:responseData?.code,hasData:!!responseData?.data,dataType:typeof responseData?.data,dataKeys:responseData?.data?Object.keys(responseData.data):[]},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'B'})}).catch(()=>{});
        }
        // #endregion
        ...
        // #region agent log
        if (url.includes("/api/graph/") || url.includes("/api/ref/")) {
            fetch('http://127.0.0.1:7242/ingest/51c402ed-a6bc-49c1-9938-f2186d8909a5',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'fetch.ts:78',message:'Graph/Backlink API error',data:{url,error:e.message,errorType:e.name},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'E'})}).catch(()=>{});
        }
        // #endregion
